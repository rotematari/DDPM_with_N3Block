...
# def im2col(input_tensor, kernel_size, stride):
#     return F.unfold(input_tensor, kernel_size, stride=stride)

# # Custom col2im implementation with overlap counts
# def col2im(input_tensor, output_shape, kernel_size, stride):
#     output = F.fold(input_tensor, output_shape[2:], kernel_size, stride=stride)
#     counts = F.fold(F.unfold(torch.ones(output_shape), kernel_size, stride=stride), output_shape[2:], kernel_size, stride=stride)
#     output /= counts
#     return output

# PyTorch-based matmul1 and its gradient implementations
def matmul1(mat_x, mat_y, mat_i, out, n, m, e, o, b):
    for batch in range(b):
        for row in range(m):
            for col in range(o):
                sum_val = 0.0
                xind_col = mat_i[batch, row, col].item()
                for i in range(e):
                    pos_y = mat_y[batch, row, i]
                    pos_x = mat_x[batch, xind_col, i]
                    sum_val += pos_y * pos_x
                out[batch, row, col] = sum_val
    return out

def matmul1_bwd(gradients, mat_x, mat_y, mat_i, out_x, out_y, m, n, e, o, b):
    # For xgrad
    for batch in range(b):
        for row in range(m):
            for col in range(o):
                pos_i = mat_i[batch, row, col].item()
                g = gradients[batch, row, col].item()
                for j in range(e):
                    pos_y = mat_y[batch, row, j]
                    pos_ox = out_x[batch, pos_i, j]
                    pos_ox += pos_y * g
                    out_x[batch, pos_i, j] = pos_ox
    ...
    for batch in range(b):
        for row in range(m):
            for col in range(e):
                sum_val = 0.0
                for i in range(o):
                    xind = mat_i[batch, row, i].item()
                    pos_x = mat_x[batch, xind, col]
                    g = gradients[batch, row, i].item()
                    sum_val += pos_x * g
                out_y[batch, row, col] = sum_val
    return out_x, out_y
'''
...
from src_denoising.models.pyinn.pyinn.im2col import im2col,col2im
import torch
import torch.nn.functional as F
import numpy as np
...
class IndexedMatmul1Efficient(torch.autograd.Function):
    ...
    def backward(ctx, grad):
        ...
        matmul1_bwd(grad,x,y,I,grad_x,grad_y, m, n, e, o, b)
...
def im2patch(x, patchsize, stride, padding=None, returnpadding=False):
    ...
    # Commented out pyinn function
    x2col = im2col(xpad, [patchsize]*2, [stride]*2, [0,0])
    
    # Replacement using PyTorch's unfold
    # x2col = F.unfold(xpad, kernel_size=(patchsize, patchsize), stride=(stride, stride))
...
def patch2im(x_patch, patchsize, stride, padding):
    ...
    # Commented out pyinn functions
    counts = col2im(torch.ones_like(x_patch), [patchsize]*2, [stride]*2, [0,0])
    x = col2im(x_patch.contiguous(), [patchsize]*2, [stride]*2, [0,0])
    
    # Replacement using PyTorch's fold
    # counts = F.fold(torch.ones_like(x_patch), output_size=(x_patch.size(2), x_patch.size(3)), kernel_size=(patchsize, patchsize), stride=(stride, stride))
    # x = F.fold(x_patch, output_size=(x_patch.size(2), x_patch.size(3)), kernel_size=(patchsize, patchsize), stride=(stride, stride))
